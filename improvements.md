...
1.  Instead of `Map<EntityId, ComponentCollection>`, group entities that have the _exact same_ set of components into an **Archetype**.
...
2.  Use a "Generation" index to handle ID recycling (so you don't reference destroyed entities).
    - _Structure:_ `EntityID = (Generation << 20) | Index`.
...
**Current:** `world.locate` and `world.findAll` iterate over _all_ entities every time they are called.
**Proposed:**
...
**Improvement:** It creates friction. If `CT` represents _all_ components, updating the union type every time you add a component is tedious.
